...
async def learn_group_messages(chat_id: int, messages: List[Dict]) -> bool:
    ...
    if messages_data:
        # Update the chat document with learned messages
...
async def get_learned_messages(chat_id: int, limit: int = 100) -> List[Dict]:
    ...
    return chat["learned_messages"][-limit:]  # Return latest messages
...
async def process_learn_command(chat_id: int, admin_id: int, client) -> str:
    ...
    if not await is_served_chat(chat_id):
        return "❌ Pehle is group ko database mein add karo!"
    ...
    try:
        ...
        if success:
            return f"✅ {len(messages)} messages successfully saved in group database!"
        return "❌ Koi valid messages nahi mile learn karne ko"
